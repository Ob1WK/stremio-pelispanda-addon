...
import { MitorrentClient } from './mitorrent-client.js';
...
export function createAddon({ name = 'streaMX', id = 'com.streamx.addon', client, novaClient, ttlSeconds = 300, logger = console } = {}) {
  ...
  const builder = new addonBuilder({
    ...
    version: '2.5.0',
    name,
    description: 'Streams de PelisPanda, MiTorrent, NOVA y Embed69, con catálogos NOVA integrados',
...
export function createFromEnv(env = process.env) {
  ...
  const mitorrentClient = env.MITORRENT_ENABLED === 'false' ? null : new MitorrentClient({
    baseUrl: env.MITORRENT_BASE_URL || 'https://mitorrent.mx/',
    metadataUrl: env.METADATA_API_URL || 'https://v3-cinemeta.strem.io/',
    maxResponseBytes: Number(env.MITORRENT_MAX_RESPONSE_BYTES) || 2 * 1024 * 1024
  });
  const clients = [pelisPandaClient, mitorrentClient, novaClient, embed69Client].filter(Boolean);
